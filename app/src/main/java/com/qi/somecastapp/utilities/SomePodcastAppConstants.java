package com.qi.somecastapp.utilities;

public class SomePodcastAppConstants {
    public static String KEY_EPISODE_META = "episode_meta";
    public static String KEY_EPISODE_TITLE = "episode_TITLE";
    public static String KEY_EPISODE_ALBUM = "episode_ALBUM";
    public static String KEY_EPISODE_DURATION = "episode_DURATION";
    public static String KEY_EPISODE_ARTIST = "episode_ARTIST";
    public static String KEY_EPISODE_ID = "episode_ID";
}
